...
//!
//! Implements the exact Polyglot specification for Zobrist hashing,
//! which differs from Lemonate's internal Zobrist implementation.
//!
//! The Polyglot format uses 781 pseudo-random 64-bit values:
//! - 768 for pieces (12 piece types × 64 squares)
//! - 4 for castling rights (KQkq)
//! - 8 for en passant files (a-h)
//! - 1 for side to move
//!
//! Piece type encoding is color-interleaved:
//! 0=black pawn, 1=white pawn, 2=black knight, 3=white knight, etc.

use crate::types::{Color, PieceType, Square};
use crate::Board;

/// The 781 Polyglot random values from the official specification.
#[rustfmt::skip]
...
///
/// This hash differs from Lemonate's internal Zobrist hash and must be used
/// when probing Polyglot opening books.
pub fn polyglot_hash(board: &Board) -> u64 {
    let mut hash = 0u64;

    // Hash pieces using Polyglot's color-interleaved piece indexing
    for color in [Color::Black, Color::White] {
        for piece_type in [
            ...
            let poly_piece = polyglot_piece_index(piece_type, color);
            let mut bb = board.piece_bitboard(color, piece_type);
            while let Some(sq) = bb.pop_lsb() {
                let index = 64 * poly_piece + 8 * sq.rank() as usize + sq.file() as usize;
    ...
    // Hash en passant (ONLY if capture is actually possible)
    if let Some(ep_sq) = board.en_passant_square() {
        if can_capture_en_passant(board, ep_sq) {
            hash ^= POLYGLOT_RANDOM[772 + ep_sq.file() as usize];
        }
    }

    // Hash side to move (Polyglot XORs this only when white to move)
...
/// Get the Polyglot piece type index for a piece.
///
/// Polyglot uses color-interleaved indexing:
/// 0=black pawn, 1=white pawn, 2=black knight, 3=white knight, etc.
#[inline]
...
///
/// Polyglot only includes the en passant file in the hash if there's
/// actually a pawn that can make the capture.
fn can_capture_en_passant(board: &Board, ep_sq: Square) -> bool {
    let side = board.side_to_move();
    let pawn_rank = if side == Color::White { 4 } else { 3 };
    let ep_file = ep_sq.file();

    let our_pawns = board.piece_bitboard(side, PieceType::Pawn);

    // Check for pawn on adjacent file that can capture
    let can_from_left =
        ep_file > 0 && our_pawns.is_set(Square::from_coords(ep_file - 1, pawn_rank));
    let can_from_right =
        ep_file < 7 && our_pawns.is_set(Square::from_coords(ep_file + 1, pawn_rank));

    can_from_left || can_from_right
}
...
mod tests {
    ...
    fn test_starting_position_hash() {
        ...
        // This value is from the Polyglot specification
    ...
    #[test]
    fn test_position_after_e4_d5() {
        let board =
            Board::from_fen("rnbqkbnr/ppp1pppp/8/3p4/4P3/8/PPPP1PPP/RNBQKBNR w KQkq d6 0 2")
                .unwrap();
        let hash = polyglot_hash(&board);

        // Known Polyglot hash for position after 1. e4 d5
        assert_eq!(hash, 0x0756B94461C50FB0);
    }

    #[test]
    fn test_position_after_e4_e5() {
        let board =
            Board::from_fen("rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPP1PPP/RNBQKBNR w KQkq e6 0 2")
                .unwrap();
        let hash = polyglot_hash(&board);

        // Known Polyglot hash for position after 1. e4 e5
        assert_eq!(hash, 0x0844931A6EF4B9A0);
    }

    #[test]
    fn test_polyglot_piece_index() {
        assert_eq!(polyglot_piece_index(PieceType::Pawn, Color::Black), 0);
        assert_eq!(polyglot_piece_index(PieceType::Pawn, Color::White), 1);
        assert_eq!(polyglot_piece_index(PieceType::Knight, Color::Black), 2);
        assert_eq!(polyglot_piece_index(PieceType::Knight, Color::White), 3);
        assert_eq!(polyglot_piece_index(PieceType::King, Color::Black), 10);
        assert_eq!(polyglot_piece_index(PieceType::King, Color::White), 11);
    }
}
